...
async def lifespan(app: FastAPI) -> AsyncGenerator[None, None]:
    ...
    if is_sqlite:
        # For SQLite (local dev), tables are created by seed.py via Base.metadata.create_all.
        # Only add missing columns using SQLite-compatible syntax (no IF NOT EXISTS for ALTER TABLE).
        async with engine.begin() as conn:
            for col_sql in [
                "ALTER TABLE users ADD COLUMN avatar VARCHAR(50)",
                "ALTER TABLE users ADD COLUMN can_create_case BOOLEAN NOT NULL DEFAULT 0",
                "ALTER TABLE users ADD COLUMN can_delete_case BOOLEAN NOT NULL DEFAULT 0",
                "ALTER TABLE visit_records ADD COLUMN case_profile_id VARCHAR REFERENCES case_profiles(id)",
                "ALTER TABLE case_profiles ADD COLUMN case_number VARCHAR(50)",
                "ALTER TABLE case_profiles ADD COLUMN city VARCHAR(50)",
                "ALTER TABLE case_profiles ADD COLUMN service_start_date VARCHAR(20)",
                "ALTER TABLE case_profiles ADD COLUMN home_service_worker VARCHAR(100)",
            ]:
                try:
        ...
        # PostgreSQL — use full migration SQL
        async with engine.begin() as conn:
            ...
            """))
            await conn.execute(text(
                "ALTER TABLE case_profiles ADD COLUMN IF NOT EXISTS case_number VARCHAR(50)"
            ))
            await conn.execute(text(
                "ALTER TABLE case_profiles ADD COLUMN IF NOT EXISTS city VARCHAR(50)"
            ))
            await conn.execute(text(
                "ALTER TABLE case_profiles ADD COLUMN IF NOT EXISTS service_start_date VARCHAR(20)"
            ))
            await conn.execute(text(
                "ALTER TABLE case_profiles ADD COLUMN IF NOT EXISTS home_service_worker VARCHAR(100)"
            ))
            await conn.execute(text("""
...
app = FastAPI(title="CareSync 長照家電訪管理系統", lifespan=lifespan)
